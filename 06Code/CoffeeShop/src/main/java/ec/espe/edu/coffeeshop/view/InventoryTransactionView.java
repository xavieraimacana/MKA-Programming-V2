...
public class InventoryTransactionView extends JFrame {
    ...
    public InventoryTransactionView() {
        ...
        setTitle("Gestión de Transacciones de Inventario - Ver Todos");
        setSize(900, 700);
        ...
        String[] columns = {"ID", "Fecha", "Tipo", "Cambio Cantidad", "Doc Ref", "Artículo"};
        tableModel = new DefaultTableModel(columns, 0);
        table = new JTable(tableModel);
        JPanel topPanel = new JPanel(new BorderLayout());
        topPanel.setBackground(ThemeManager.COLOR_WHITE);
        JLabel titleLabel = new JLabel("TRANSACCIONES DE INVENTARIO - SÓLO LECTURA");
        ...
        btnAdd.addActionListener(e -> new InventoryTransactionAddView(this, controller).setVisible(true));
    ...
    public void loadTableData() {
        ...
        List<InventoryTransaction> txs = controller.getAllTransactions();
        for (InventoryTransaction tx : txs) {
            tableModel.addRow(new Object[]{
                    tx.getTransactionId(),
                    tx.getDate() != null ? tx.getDate().format(formatter) : "",
                    tx.getType().toString(),
                    String.format("%.2f", tx.getQuantityChange()),
                    tx.getReferenceDocumentId(),
